...
class UFine6926(BaseDataset):
    ...
    [{'split', str,
      ...
      'id', int}...]
    """
    dataset_dir = 'UFine6926'

    def __init__(self, args, root='', verbose=True):
        super(UFine6926, self).__init__()
        self.dataset_dir = op.join(root, self.dataset_dir)
        self.img_dir = op.join(self.dataset_dir, 'imgs/')

        self.anno_path = op.join(self.dataset_dir, 'one-shot-semi.json')
        ...
        if verbose:
            self.logger.info("=> UFine6926 Images and Captions are loaded")
