...
def read_ulab_image(args, p_uids, ulab_id_path):
    train_transforms = build_transforms(img_size=args.img_size,
                                        aug=args.img_aug,
                                        is_train=True)
    batch_size = p_uids.shape[0]
    p_uids = p_uids.cpu().numpy()
    for i in range(batch_size):
        pdb.set_trace()
        img_path1 = ulab_id_path(p_uids[i,0])
        img1 = train_transforms(read_image(img_path1))
        
        img_path2 = ulab_id_path(p_uids[i,1])
        img2 = train_transforms(read_image(img_path2))
        
        img_path3 = ulab_id_path(p_uids[i,2])
        img3 = train_transforms(read_image(img_path3))
...
def build_dataloader(args, tranforms=None):
    ...
    dataset = __factory[args.dataset_name](args, root=args.root_dir)
    ...
    if args.training:
        train_transforms = build_transforms(img_size=args.img_size,
                                            aug=args.img_aug,
                                            is_train=True)
        auto_transforms = build_transforms(img_size=args.img_size,
                                            aug=False,
                                            is_train=False)
        ...
        if args.MLM:
            train_set = ImageTextMLMDataset(args,
                                     dataset.train,
                                     train_transforms,
        ...
        ds_unlab = dataset.unlabel
        unlab_img_set = ImageDataset(ds_unlab['image_pids'], ds_unlab['img_paths'],
                                   val_transforms)

        unlab_img_loader = DataLoader(unlab_img_set,
                                    batch_size=args.batch_size,
                                    shuffle=False,
                                    num_workers=num_workers)
                                    
        ds_train = np.array(dataset.train)
        #pdb.set_trace()
        ptrain_img_set = ImageDataset(ds_train[:,1], ds_train[:,2],
                                   val_transforms)
        ptrain_txt_set = TextDataset(ds_train[:,1], ds_train[:,3],
                                  text_length=args.text_length)

        ptrain_img_loader = DataLoader(ptrain_img_set,
                                    batch_size=args.batch_size,
                                    shuffle=False,
                                    num_workers=num_workers)
        ptrain_txt_loader = DataLoader(ptrain_txt_set,
                                    batch_size=args.batch_size,
                                    shuffle=False,
                                    num_workers=num_workers)

        return train_loader, val_img_loader, val_txt_loader, unlab_img_loader, ptrain_img_loader, ptrain_txt_loader, num_classes, dataset.unlab_id_path
